# AtlasNet Unity: local backend demos
...
In Package Manager, select **AtlasNet Unity** and import **Simple Authority Demo**. The sample's scripts, prefabs, and scenes are editable project assets after import. The runtime package stays in `Packages`, not in the project's `Assets` directory. The sample keeps its existing import name but now focuses on the local multi-worker backend. It uses Unity's legacy `Input` API to keep the controller readable; set **Active Input Handling** to **Both** or **Input Manager** in Player Settings. The package API itself does not require that input system.

## Run with Multiplayer Play Mode

1. Open `ClientCrossServer`, `ServerCrossServer`, or `PhysicsDemo` from the imported sample in every instance. Install **Multiplayer Play Mode 3.0.0** and start the main Editor Player plus at least two additional editor instances. This project has that package in its manifest. For the larger multi-worker `ScaleDemo` flow, see `LocalWorkers.md`.
2. In the main Game view, click **Start Host**. In a second Game view, click **Join as Worker**. In each further Game view, click **Join as Client**. Alternatively, use **Start Server Only** in one view, a worker in another, and clients in the remaining views. All instances use `127.0.0.1:7777`; no gameplay script edits are needed.
...
4. A newly joined client should receive existing visible entities and the latest persistent value. It receives every entity authored by its player's current server, plus nearby entities authored by other servers when its player has `NetworkInterestSource`. `PhysicsDemo` now uses that same local multi-worker interest and handoff path. Disconnecting a client despawns its owned player canonically. `HideFrom` only removes a particular client's observer copy; it is not canonical deletion.

For a standalone smoke test, the sample launcher also accepts `-atlas-host`, `-atlas-server`, `-atlas-worker`, or `-atlas-client`. Add `-atlas-scene=ClientCrossServer`, `-atlas-scene=ServerCrossServer`, `-atlas-scene=ScaleDemo`, or `-atlas-scene=PhysicsDemo` to load that built scene before starting the role. `-atlas-test-rpc` makes the local player request the sample action once, and `-atlas-metrics` writes a server metric line every five seconds. These switches belong only to the sample launcher, not to the framework API.

The `ClientCrossServerPlayer` prefab has owner-written position and rotation. In `ServerCrossServerPlayer`, only yaw/pitch are owner-written; the client sends movement input to the current simulation authority, which moves its CharacterController on the network tick. Both scenes enable automatic local handoff and player-radius cross-worker interest. There is deliberately **no movement prediction** in this demo, so server-authoritative movement has visible input delay. Aim remains immediate for the local owner. No artificial latency is configured.
...
The `PhysicsDemo` scene uses a separate server-authoritative `PhysicsPlayer` with a dynamic Rigidbody and CapsuleCollider. The owner sends movement intent on network ticks; its simulation worker sets horizontal velocity and jumps in `FixedUpdate`. Mouse aim rotates the owner's camera/aim pivot immediately without rotating the physics body. The scene spawns three registered Rigidbody cubes in different parts of the world. Only each body's current simulation worker runs dynamic physics; other copies are kinematic. Walk into a cube on the **same worker** to test ordinary physics pushing. Press **R in a server/worker Game view** to kick only cubes authoritative in that window. Both players and cubes can hand off at region boundaries, carrying linear and angular velocity. The top-down server camera, region overlay, and player interest disk help inspect this. This is not distributed collision: a player cannot physically push a cube owned by a different worker through its kinematic ghost. Movement prediction is not included. Test all of this in Multiplayer Play Mode before treating it as validated.
...
The active authority examples now use the local multi-worker backend to exercise worker ghosts, bounded worker-interest delivery, routing, and region handoffs. This path has not had a multi-instance Unity runtime validation pass; see `LocalWorkers.md`. The older single-server authority scenes and prefabs were retired. The local backend does not provide reconnect/resume, production transport tuning, encryption, prediction, rollback, lag compensation, distributed physics, or the C++ AtlasNet integration.

The `PhysicsDemo` player and cubes use server-simulated 3D Rigidbodies; the other player examples still use `CharacterController`. Rigidbody handoff and pushing are wired but await Play Mode validation. Distributed collision and predicted physics remain outside the demonstrated scope.
...
Before claiming the demo complete, exercise a non-host client and a worker in both cross-server authority scenes, the three RPC destinations, late join, spawn/despawn, the scale scene, and a package install in a separate clean project. EditMode asset checks alone cannot verify Play Mode behavior.
