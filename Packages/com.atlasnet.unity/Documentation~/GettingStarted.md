...
1. Open `ClientAuthority`, `ServerAuthority`, `ScaleDemo`, or `PhysicsDemo` from the imported sample. Install **Multiplayer Play Mode 3.0.0** and start the main Editor Player plus at least one additional editor instance. This project has that package in its manifest.
...
For a standalone smoke test, the sample launcher also accepts `-atlas-host`, `-atlas-server`, or `-atlas-client`. Add `-atlas-scene=ServerAuthority`, `-atlas-scene=ScaleDemo`, or `-atlas-scene=PhysicsDemo` to load that built scene before starting the role. `-atlas-test-rpc` makes the local player request the sample action once, and `-atlas-metrics` writes a server metric line every five seconds. These switches belong only to the sample launcher, not to the framework API.
