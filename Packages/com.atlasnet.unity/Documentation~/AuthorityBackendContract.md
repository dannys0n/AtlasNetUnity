...
The current package uses `LocalVoronoi` for placement and a small local radius policy for interest. `NetworkManager.LocalWorkers` performs export, prepare, commit, routing, and packet forwarding through the **first Unity server** as coordinator and ingress. Other Unity workers connect to it rather than to one another. This is deliberately not the intended AtlasNet node-to-node packet path. It remains useful for exercising stable IDs, permissions, epochs, snapshots, ghost lifecycle, and developer-facing gameplay code.

The coordinator now separates its global entity directory from its Unity replicas. The directory contains ID, prefab key, controlling session, authority worker/epoch, last pose, replication schema, and cached public/owner snapshots. `SpawnedObjects` and `TryGet` describe **locally resident** Unity objects only. After a handoff, the coordinator removes an out-of-interest ghost; it may temporarily recreate one while transferring simulation and owner-written state. A host may also retain a replica for its own client view. A coordinator directory entry outliving a local GameObject is expected, not hidden simulation authority. Only the assigned worker should run simulation logic.

For local late joins, authoritative workers currently send full cached-state updates to the coordinator. That is a development relay mechanism and can add substantial loopback traffic in the scaling scene; it is not the proposed AtlasNet backend wire format or a performance target. Native integration should provide authoritative snapshots/subscription deltas and routing through its own contract, not copy these packet types.
...
The Unity runtime should eventually consume assignment, residency, routing-result, and handoff notifications through an internal bridge. The local directory/replica split and shared client/worker replica creation path move Unity toward that boundary, but the coordinator, local interest policy, and packet dispatch are still in `NetworkManager`; merely replacing `LocalTcpTransport` with another byte transport would **not** be a complete native integration. Do not infer a P/Invoke ABI, client wire format, or C++ feature availability from local packet numbers, `NetworkInterestSource`, or debug polygons.
