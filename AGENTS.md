...
For the future backend division, use `Packages/com.atlasnet.unity/Documentation~/AuthorityBackendContract.md`: AtlasNet is expected to own simulation assignment, entity-directed routing, handoff decisions/commit, and replica residency notifications. The exact interest/filtering contract is still open, and the current C++ branch does not yet implement the Unity lifecycle contract. The localhost implementation stands in for it with player-radius worker interest: regions find candidate owners, then entity distance decides replicas. The first server keeps a lightweight global entity directory; its Unity `NetworkObject` instances should be only local authority, local interest, host-client interest, or temporary handoff state. Region geometry is optional debug data, never gameplay authority. Do not confuse worker interest with client visibility or line of sight.
